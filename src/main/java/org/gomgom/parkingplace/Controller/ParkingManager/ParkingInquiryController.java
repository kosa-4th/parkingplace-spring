...
public class ParkingInquiryController {
    ...
    public ResponseEntity<InquiryDto.ParkingInquiryResponseDto> getParkingInquiries(InquiryDto.ParkingInquiryRequestDto requestDto,
                                                                                    Pageable pageable,
                                                                                    @AuthenticationPrincipal CustomUserDetails userDetails) {
        log.info("Controller: 주차장 관리자 문의 목록 불러오기");
        return ResponseEntity.ok(inquiryService.getInquiriesByParking(userDetails.getUser(), requestDto, pageable));
    }
}
